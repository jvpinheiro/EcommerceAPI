...
application.config['SECRET_KEY'] = 'vitor1009'
...
def add_to_cart(product_id):
    ...
    if user and product:
        cart_item = CartItem(product_id=product.id, user_id=user.id)

        db.session.add(cart_item)
        db.session.commit()
        return jsonify({"message": "Item added to cart successfully!"})
    
    return jsonify({"message": "Failed to add item in the cart."}), 400
